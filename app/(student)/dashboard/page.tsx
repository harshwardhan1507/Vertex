...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const [userRes, scoreRes, eventsRes, regRes, certRes] = await Promise.all([
      ...
      if (regRes.data) setReminders(regRes.data)
  ...
  const rank = score ? Math.ceil((1 - ((score.percentile || 0) / 100)) * 100) : '--'
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 lg:pb-0">
      ...
      <div className="mb-6 lg:mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--color-text-muted)] text-sm mt-1 lg:mt-2">
             {events.length} events this week · Your VScore is looking great!
          </p>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          <div className="w-[38px] h-[38px] rounded-full bg-[var(--color-surface)] border border-[var(--color-border-subtle)] flex items-center justify-center cursor-pointer hover:bg-[var(--color-border-hover)] transition-colors text-amber-500 shadow-sm relative">
            ...
            {reminders.length > 0 && <div className="absolute top-0.5 right-1 w-[8px] h-[8px] bg-red-500 rounded-full border-[2px] border-[var(--color-surface)]" />}
          </div>
      ...
      <div className="hidden lg:grid grid-cols-4 gap-4 mb-7">
        ...
        <div className="bg-card-grad border border-[var(--color-border-subtle)] rounded-[14px] p-5 relative overflow-hidden transition-all hover:border-[var(--color-border-hover)] hover:-translate-y-0.5">
          ...
          <div className="font-headline text-[28px] font-extrabold leading-none mb-1 tracking-[-0.03em]">{reminders.length || 0}</div>
          <div className="text-[12px] text-[var(--color-text-muted)]">Active Registrations</div>
        </div>
      </div>
      ...
      <section className="mb-8 relative z-10 w-full">
          ...
          <div className="flex items-center gap-6 relative z-10">
            ...
            <div className="flex-1 space-y-4">
              <div>
                ...
                <span className="text-xs text-[var(--color-text-muted)]">Top {rank === '--' ? 100 : rank}% of campus</span>
              </div>
        ...
        <div className="hidden lg:flex vscore-banner-bg border border-[rgba(124,92,252,0.3)] rounded-2xl p-6 lg:p-6 lg:px-7 items-center gap-6 lg:gap-7 relative overflow-hidden shadow-xl shadow-[rgba(124,92,252,0.05)]">
          ...
          <div className="flex-1">
            ...
            <div className="text-[12px] text-[var(--color-text-muted)] mb-[14px]">Top {rank === '--' ? 100 : rank}% of campus · Sem {user?.semester || 1}, {new Date().getFullYear()}</div>
        ...
        <div className="mb-8 lg:mb-0">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2">
            <div className="font-headline font-bold text-lg text-[var(--color-text-main)] flex items-center gap-2">
              Upcoming Events 
              <span className="bg-[rgba(124,92,252,0.15)] text-[var(--color-primary)] text-[11px] font-semibold px-2 py-0.5 rounded-full">{events.length} open</span>
            </div>
            <Link href="/events" className="text-[var(--color-primary)] text-xs font-bold hover:opacity-70 transition-opacity">View all →</Link>
          </div>

          {/* Event Cards List */}
          <div className="space-y-3">
            {events.length === 0 ? (
               <div className="text-[var(--color-text-muted)] text-sm py-8 bg-[var(--color-surface)] border border-[var(--color-border-subtle)] border-dashed rounded-2xl flex items-center justify-center">No upcoming events scheduled.</div>
            ) : (
              ...
              return (
              <div key={event.id} className={`group ${isFirst ? 'nebula-card border-[rgba(124,92,252,0.3)] shadow-[0_10px_40px_rgba(124,92,252,0.1)]' : 'card-grad'} border-[var(--color-border-subtle)] hover:border-[var(--color-border-hover)] border rounded-2xl lg:rounded-[14px] p-[18px] lg:px-5 flex gap-4 items-start cursor-pointer transition-all`}>
                <div className={`flex flex-col items-center justify-center w-[54px] h-[58px] ${isFirst ? 'bg-[var(--color-surface)] shadow-sm border border-[var(--color-border-subtle)]' : 'bg-[var(--color-border-subtle)] border border-[var(--color-border-subtle)]'} rounded-[12px] flex-shrink-0`}>
                  <div className={`font-headline text-[22px] font-extrabold leading-none ${isFirst ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-main)]'} text-center w-full mt-1`}>{day}</div>
                  <div className={`text-[10px] ${isFirst ? 'text-[var(--color-text-main)]' : 'text-[var(--color-text-muted)]'} tracking-[0.06em] mt-0.5 uppercase text-center w-full font-bold`}>{month}</div>
                </div>
        ...
        <div className="flex flex-col gap-5 lg:gap-5">
          ...
          <div className="lg:card-grad lg:border lg:border-[var(--color-border-subtle)] lg:rounded-[14px] lg:p-5">
            ...
            <div className="flex flex-col gap-3 lg:gap-0 lg:divide-y divide-[var(--color-border-subtle)]">
              {reminders.length === 0 ? (
                <div className="text-[var(--color-text-muted)] text-[12px] py-2 lg:py-2.5">No active event registrations.</div>
              ) : reminders.map((reg, idx) => {
                 const evtDate = new Date(reg.events.start_time);
                 const dateStr = evtDate.toLocaleString('default', { month: 'short', day: 'numeric' });
                 return (
                  <Link href={`/events/${reg.event_id}`} key={idx} className="flex items-center gap-3 lg:gap-3 p-4 lg:p-0 lg:py-3 hover:bg-[var(--color-surface)] lg:hover:bg-transparent rounded-2xl lg:rounded-none border border-[var(--color-border-subtle)] lg:border-none transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold lg:font-semibold text-sm lg:text-[12px] truncate text-[var(--color-text-main)]">{reg.events.title}</h4>
                      <p className="text-[10px] text-[var(--color-text-muted)] mt-0.5">{reg.events.venue}</p>
                    </div>
          ...
          <div className="hidden lg:block panel-card bg-card-grad border border-[var(--color-border-subtle)] rounded-[14px] p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/certificates" className="text-[12px] text-[var(--color-primary)] cursor-pointer font-medium hover:opacity-70">All →</Link>
            </div>

            <div className="flex flex-col">
              {certificates.length === 0 ? (
                 <div className="text-[var(--color-text-muted)] text-[12px] py-2">No certificates earned yet.</div>
              ) : certificates.map((cert, idx) => {
                 const certDate = new Date(cert.issued_at);
                 return (
                  <Link href={`/certificates/${cert.id}`} key={idx} className="flex items-center gap-3 py-2.5 border-b border-[var(--color-border-subtle)] cursor-pointer hover:pl-1 transition-all last:border-b-0">
                    <div className="w-9 h-9 rounded-lg bg-[rgba(124,92,252,0.15)] flex items-center justify-center text-[16px] flex-shrink-0">🏆</div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] font-semibold text-[var(--color-text-main)] truncate">{cert.events?.title || 'Unknown Event'}</div>
                      <div className="text-[10px] text-[var(--color-text-muted)] mt-0.5">{certDate.toLocaleString('default', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                    </div>
